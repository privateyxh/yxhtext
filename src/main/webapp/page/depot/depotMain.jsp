	...
	<div class="panel-body" style="padding-bottom: 0px;">
		<div id="toolbar" class="btn-group">
			<button id="btn_add" type="button" class="btn btn-default">
				<span class="glyphicon glyphicon-plus" aria-hidden="true"></span>添加仓库资料
			</button>
		...
		<div class="modal-dialog" role="document">
			<div class="modal-content">
				...
				<div class="modal-body">
					<form action="${pageContext.request.contextPath}/depot/add.action"
						method="post">
						<div class="form-group">
							<label for="txt_departmentname">ID</label> <input type="text"
								name="depotId" class="form-control" id="txt_departmentname"
								placeholder="ID">
						</div>
						<div class="form-group">
							<label for="txt_departmentname">仓库名</label> <input type="text"
								name="depotName" class="form-control" id="txt_departmentname"
								placeholder="仓库名">
						</div>
						<div class="form-group">
							<label for="txt_parentdepartment">地址</label> <input type="text"
								name="depotAddress" class="form-control"
								id="txt_parentdepartment" placeholder="地址">
						</div>
						<div class="form-group">
							<label for="txt_departmentlevel">联系人</label> <input type="text"
								name="depotUser" class="form-control" id="txt_departmentlevel"
								placeholder="联系人">
						</div>
						<div class="form-group">
							<label for="txt_departmentlevel">联系人电话</label> <input type="text"
								name="depotUserPhone" class="form-control"
								id="txt_departmentlevel" placeholder="联系人电话">
						</div>
						<div class="form-group">
							<label for="txt_departmentlevel">状态</label><br /> 
							<select name="depotState">
								<option value="0">运营中</option>
								<option value="1">停止运营(将会停止运行)</option>
							</select>
						</div>
						<div class="form-group">
							<label for="txt_departmentlevel">备注</label> <input type="text"
								name="depotExplain" class="form-control"
...
var TableInit = function () {
    ...
    oTableInit.Init = function () {
        $('#tb_departments').bootstrapTable({
            url: '${pageContext.request.contextPath}/depot/page.action',         //请求后台的URL（*）
            method: 'get',                      //请求方式（*）
            ...
            uniqueId: "DEPOTID",                     //每一行的唯一标识，一般为主键列
            ...
            columns: [{
            	...
            	var data = {  
                    	...
                    	depotAddress:row.DEPOTADDRESS,
                    	depotUser:row.DEPOTUSER,
                    	depotUserPhone:row.DEPOTUSERPHONE,
                    	depotState:row.DEPOTSTATE,
                    	depotExplain:row.DEPOTEXPLAIN
                }  
                $.ajax({
                    type: "post",
                    url: "${pageContext.request.contextPath}/depot/update.action",
                    type : "POST",  
...
$("#btn_delete").click(function () {
	...
	function deleteUser(ids) {  
	    ...
	    if (confirm(msg) == true) {  
	        $.ajax({  
	            url: "${pageContext.request.contextPath}/depot/del.action",  
	            type : "POST",  
                data : ids,
	            success: function (data) {  
	                ...
	                window.location.href="${pageContext.request.contextPath}/page/depot/depotMain.jsp";
	            }  
	        });
	    }  
	}   
