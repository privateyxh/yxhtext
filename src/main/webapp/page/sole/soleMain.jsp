...
<body>
	...
	<div class="panel-body" style="padding-bottom: 0px;">
		<div id="toolbar" class="btn-group">
			<button id="btn_add" type="button" class="btn btn-default">
				<span class="glyphicon glyphicon-plus" aria-hidden="true"></span>添加订单信息
			</button>
		</div>
		...
		<div class="modal-dialog" role="document">
			<div class="modal-content">
				...
				<div class="modal-body">
					<form action="${pageContext.request.contextPath}/sole/addbysole.action"
						method="post">
						<div class="form-group">
							<label for="txt_departmentname">ID</label> <input type="text"
								name="soleId" class="form-control" id="txt_departmentname"
								placeholder="ID">
						</div>
						<div class="form-group">
							<label for="txt_departmentname">购买单位ID</label>
						...
						<div class="form-group">
							<label for="txt_departmentlevel">单据备注</label> 
							<input type="text"
								name="soleExplain" class="form-control" id="txt_departmentname"
								placeholder="ID">
						</div>
						<div class="form-group">
							<label for="txt_departmentlevel">商品名</label> 
							<select class="form-control"  name="goodName">
	...
	<script type="text/javascript">
		...
		var TableInit = function() {
			...
			oTableInit.Init = function() {
				$('#tb_departments')
						.bootstrapTable(
								{
									url : '${pageContext.request.contextPath}/sole/page.action', //请求后台的URL（*）
									method : 'get', //请求方式（*）
									...
									onEditableSave : function(field, row,
										...
										var data = {
												soleId : row.SOLEID,
												soleDate : row.SOLEDATE,
												companyId : row.COMPANYID,
												userId : row.USERID,
												goodId : row.GOODID,
												goodName : row.GOODNAME,
												goodNumber : row.GOODNUMBER,
												soleExplain : row.SOLEEXPLAIN
										}
										$
												.ajax({
													type : "post",
													url : "${pageContext.request.contextPath}/sole/update.action",
													type : "POST",
		...
		function getCompanyinfo() {
			$
					.ajax({
						url : "${pageContext.request.contextPath}/sole/findbycompany.action",
						type : "POST",
						success : function(data) {
		...
		function getGoodsinfo() {
			$
					.ajax({
						url : "${pageContext.request.contextPath}/sole/findbygoods.action",
						type : "POST",
						success : function(data) {
